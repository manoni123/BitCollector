...
namespace Assets.FantasyInventory.Scripts.Interface.Elements
{
    ...
    public class ScrollShop : ShopItemContainer
    {
        ...
        private void Reset()
        {
            ...
            if (Grid.constraint == GridLayoutGroup.Constraint.FixedColumnCount && MinRows > 0)
            {
                ...
                var width = columns * Grid.cellSize.x + (columns - 1) * Grid.spacing.x;
                var height = rows * Grid.cellSize.y + (rows - 1) * Grid.spacing.y;
